...
@onready var pineapple_label: Label = $Pineapple
...
@onready var green_checkmark: Sprite2D = $Green/GreenCheckmark
@onready var pineapple_checkmark: Sprite2D = $Pineapple/PineappleCheckmark
...
@onready var pineapple_sprite: Sprite2D = $PineappleSprite
...
func setSpecificEquations(pepEq, mushEq, onionEq, greenEq, pineEq):
	pepperoni_label.text = pepEq
	mushroom_label.text = mushEq
	onion_label.text = onionEq
	green_label.text = greenEq
	pineapple_label.text = pineEq
	pizzaMode.emit()
	resetCheckmarks()
	setCheckmarks()
	setSprites()
...
func setSprites() -> void:
	...
	if (green_label.text == ""):
		...
		green_sprite.show()
	if (pineapple_label.text == ""):
		pineapple_sprite.hide()
	else:
		pineapple_sprite.show()
...
func setCheckmarks() -> void:
	...
	if (onion_label.text == ""):
		...
		#onion_sprite.show()
	if (green_label.text == ""):
		green_checkmark.texture = null
	if (pineapple_label.text == ""):
		pineapple_checkmark.texture = null
...
func totalToppings() -> int:
	if not (pepperoni_label.text == "" and mushroom_label.text == "" and onion_label.text == "" and green_label.text == "" and pineapple_label.text == ""):
		var total: int = 0
		var expression = Expression.new()
		expression.parse(pepperoni_label.text)
		var result = expression.execute()
		total += result
		expression = Expression.new()
		expression.parse(mushroom_label.text)
		result = expression.execute()
		total += result
		expression = Expression.new()
		expression.parse(onion_label.text)
		result = expression.execute()
		total += result
		expression = Expression.new()
		expression.parse(green_label.text)
		result = expression.execute()
		total += result
		expression = Expression.new()
		expression.parse(pineapple_label.text)
		result = expression.execute()
		total += result
		return total
	else:
...
func _on_Line_Edit_text_entered(new_text: String) -> bool:
	if (new_text == "r"):
		resetEquations()
		return true
	elif not (pepperoni_label.text == "" and mushroom_label.text == "" and onion_label.text == ""):
		...
		var greenBool: bool = solveEquation(manager.toppings["green"], green_label)
		var pineBool: bool = solveEquation(manager.toppings["pineapple"], pineapple_label)
		if (pepBool and mushBool and onionBool and greenBool and pineBool):
			solution_label.text = "Correct!"
			...
			win_timer.start()
			winSound.emit()
			scoreSignal.emit(totalToppings())
			#leavePizzaMode.emit(true)
			#hide()
			return true
		else:
			solution_label.text = "Try again"
			loseSound.emit()
			return false
	else:
		return false
...
func _on_order_scene_game_over(score: int) -> void:
	manager.clear_all()
